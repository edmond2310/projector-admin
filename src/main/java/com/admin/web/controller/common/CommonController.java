...
//            String fileName = FileUploadUtils.upload(filePath, file);
            // 上传并返回新文件名称oss
            String extension = aliyunOssUtil.getExtension(file);
            String objectName = aliyunOssUtil.genObjectName(file);
            aliyunOssUtil.uploadFile(file.getBytes(), objectName);
            BufferedImage image;
            ...
            String url = AliyunOssConfig.getHttpUrlAccessPrefix() + objectName + "?" + image.getWidth() + "x" + image.getHeight();
//            String url = serverConfig.getUrl() + fileName;
            AjaxResult ajax = AjaxResult.success();
            ajax.put("url", url);
